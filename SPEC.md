...
> **상태: 설계 확정, 구현 전.** 0~6단계(MVP)가 끝난 뒤의 방향이다. 19장의 단계 순서대로 진행하고, 18장(도구 노드)은 **각 노드를 착수하기 전에 이 문서를 다시 고쳐** 구체화한다.

### 15.1 무엇이 바뀌는가
MVP는 **터미널 멀티플렉서**였다. 캔버스는 터미널을 배치하는 수단이었다.
v2는 **캔버스가 본체**이고 터미널은 그 위의 한 종류다. DB 관계 그래프, 코드 뷰어, 분석 도구, MCP 툴이 같은 캔버스에 노드로 올라간다.

중심축이 바뀌는 작업이므로, **단계마다 앱이 멀쩡히 쓸 수 있는 상태를 유지하는 것**을 다른 무엇보다 우선한다.

### 15.2 무엇을 캔버스에 올리는가 (결정)
**여러 개를 동시에 나란히 놓고 비교하는 것만 노드로 만든다.**

| 노드로 | 고정 패널로 |
|---|---|
| 터미널, 코드 파일, DB 관계 그래프, 분석 결과 | 파일 트리, 검색, 레이어 목록, 설정 |

"전부 노드로"는 일관성은 있지만 캔버스가 금방 복잡해진다. 기준 없이 가면 나중에 일관성이 깨진다.

### 15.3 지켜야 할 것
- **이 앱의 차별점은 tmux 영속성과 상태 감지다.** 코드 뷰어·DB 그래프는 VS Code·DataGrip이 더 잘한다. 새 노드는 *"캔버스 위에서 여러 Claude Code 세션이 도는 맥락"*과 붙을 때만 의미가 있다 (예: 그 노드의 Claude가 방금 고친 파일을 코드 노드가 강조).
- 2.2의 비범위(원격 세션, 다중 워크스페이스, 협업, 코드 서명)는 v2에서도 그대로 비범위다.
...
### 16.1 문제
지금은 **"노드 = 터미널"이 뼈대에 박혀 있다.** `nodeTypes`는 하나뿐이고, `TerminalNodeData`에 `tmuxSession`·`claudeSessionId`·`command`가 섞여 있으며, 저장 포맷도 그 타입 단일이다. 이 상태로 새 종류를 더하면 전부 터미널 가정을 물고 늘어진다.
...
type CanvasNode =
  ...
  | (BaseNodeData & { kind: 'code'; code: CodePayload })
  | ...                                          // 판별 유니온
```
- **`tmuxSession`·`claudeSessionId`·`command`는 `TerminalPayload`로 내린다.** 이것이 v2의 1순위 리팩터링이다.
- 종류가 늘어도 `BaseNodeData`를 다루는 코드(이동·리사이즈·선택·레이어·저장)는 그대로여야 한다.

### 16.3 런타임 레지스트리
노드는 무거운 자원을 쥔다 — PTY, DB 커넥션, 파일 와처, MCP 세션. 지금 `TerminalRegistry`가 터미널에 하는 일(React 밖 보관, 언마운트해도 유지, 닫을 때만 정리 — 6.4)을 종류별로 일반화한다.

```ts
interface NodeRuntime<T> {
  acquire(node, container): T
  detach(id): void          // 화면에서 뗀다. 자원은 유지
  dispose(id, mode): void   // 실제로 끝낸다
  survivesUnmount: boolean  // 개요 단계에서 언마운트해도 살아 있어야 하는가
}
```

### 16.4 저장 포맷 v2
`Workspace.version`을 **2**로 올리고, v1 파일은 **모든 노드를 `kind: 'terminal'`로** 올려 읽는다(9.2의 마이그레이션 자리). v1로 되돌리는 기능은 만들지 않는다.
...
### 17.1 모드 분리 (결정)
**이 프로젝트에만 있는 문제**: Figma는 `V`·`F`·`R`·`T` 같은 한 글자 단축키로 살지만, 우리 터미널은 **모든 키를 그대로 넘겨야 한다**(7.5).

| 모드 | 키보드 | 진입 |
|---|---|---|
| **캔버스 모드** (기본) | 한 글자 단축키 사용. 터미널은 **입력을 받지 않는다**(미리보기) | 빈 곳 클릭, 또는 노드 안에서 `Cmd+.` |
| **노드 모드** | 모든 키가 그 노드로. 앱은 `Cmd+.`만 가로챈다 | 노드 **더블클릭** |

- Figma에서 프레임 안으로 들어가는 감각과 같고, 이미 있는 **줌 단계(7.3)의 "미리보기에서는 입력 불가"를 모드로 승격**시킨 것이다.
- **`Esc`는 전환키로 쓸 수 없다.** Claude Code가 중단에 쓴다(7.5). `Cmd+.`를 쓴다.
- 지금 꺼 둔 `multiSelectionKeyCode`·`selectionKeyCode`(7.5의 주의)는 **캔버스 모드에서만** 켠다.

### 17.2 프레임(그룹)
React Flow의 `parentId` + `extent: 'parent'`를 쓴다. 프레임을 옮기면 자식이 따라오고, 프레임 단위로 접기·색·이름을 준다. 구상의 "노드를 그룹으로 나누어 관리"가 여기에 해당한다.

### 17.3 레이어 패널
노드가 20개를 넘으면 캔버스만으로는 못 찾는다. 고정 패널에 트리로 보여 주고 **z-order 변경·숨김·잠금·이름 바꾸기**를 한다. `BaseNodeData`의 `z`·`hidden`·`locked`가 그 모델이다.

### 17.4 그 밖에 가져오는 것 / 안 가져오는 것
| 가져온다 | 안 가져온다 |
|---|---|
| 다중 선택, 정렬·배치, 스냅·정렬 가이드, 텍스트 노트, 사각형·화살표 | 벡터 펜·불리언 연산, 컴포넌트/베리언트, 실시간 협업, 오토레이아웃 |

오토레이아웃은 터미널 크기가 cols/rows에 묶여 있어(6.1) 맞지 않는다.

---

## 18. 도구 노드 (개요)

> ⚠️ **각 노드를 착수하기 전에 이 장을 구체화하고 완료 기준을 정한다.** 지금은 방향만 적는다.

### 18.1 코드 노드
프로젝트 파일을 읽기 전용으로 띄운다. CodeMirror 6(Monaco보다 가볍다). git diff 표시. **편집은 Claude Code가 터미널에서 하므로, 뷰어와 diff가 핵심이고 편집기는 뒤로 미룬다.**

### 18.2 DB 관계 노드
접속 → 스키마 읽기 → 관계 그래프. 대상은 MySQL·PostgreSQL. **접속 정보는 `workspace.json`에 넣지 않는다**(11장) — macOS Keychain을 쓴다(R12).

### 18.3 MCP·분석 도구 노드
MCP 클라이언트를 띄워 도구 결과를 노드로 보여 준다. 구동 위치(main/renderer)와 권한은 R13에서 확인한다.
...
| 단계 | 내용 | 완료 기준 |
|---|---|---|
| **7** 노드 종류 추상화 | 16장 전체. `NodeKind`, 터미널을 한 종류로 격하, 저장 v2 마이그레이션, `NodeRuntimeRegistry` | **겉보기 변화 없음** — 0~6단계의 모든 점검(`verify:*`)이 그대로 통과 · v1 `workspace.json`이 손실 없이 열림 · 터미널 페이로드가 `BaseNodeData`에서 분리됨 |
| **8** 캔버스 편집 기반 | 17장 전체. 모드 분리, 다중 선택·정렬, 프레임, 레이어 패널, 노트·도형, 스냅 | 노드 20개를 프레임 3개로 묶어 관리 가능 · 캔버스 모드에서 단축키가 터미널을 방해하지 않음 · 노드 모드에서 `Esc`·`Ctrl+*`이 그대로 전달됨 · 레이어에서 숨김·잠금·순서 변경 |
| **9** 코드 노드 | 18.1 | 파일을 노드로 띄우고 여러 개 비교 · 그 프로젝트의 Claude가 고친 파일이 표시됨 |
| **10** DB 관계 노드 | 18.2 | 로컬 DB에 접속해 관계 그래프 표시 · 접속 정보가 workspace.json에 남지 않음 |
| **11** MCP·분석 도구 | 18.3 | 착수 전 정의 |

**8단계까지가 "Figma처럼"의 실체**이고, 9단계부터가 개발 툴이다.
...
| # | 항목 | 확인 방법 | 시점 |
|---|---|---|---|
| R9 | 코드 뷰어를 여러 개 띄웠을 때 성능 | 터미널 10개 + 코드 노드 5개로 부하 테스트. 지금도 WebGL을 4개로 제한 중(6.3) | 단계 9 |
| R10 | `parentId` 중첩과 우리 `measured`·리사이즈 로직의 상호작용 | 프레임 안 노드를 리사이즈해 보기. **리사이즈는 이미 한 번 크게 깨졌다**(HANDOVER 6-4) | 단계 8 시작 |
| R11 | 레이어 z-order를 React Flow가 어디까지 지원하는지 | 그룹 안 z-order를 직접 확인 | 단계 8 시작 |
| R12 | DB 접속 정보 저장 위치·암호화 | macOS Keychain 연동 방법 확인. **비밀값을 파일에 쓰지 않는다** | 단계 10 시작 |
| R13 | Electron에서 MCP 클라이언트 구동 | main/renderer 중 어디서, 권한 모델은 | 단계 11 시작 |
