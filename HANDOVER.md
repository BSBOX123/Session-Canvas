...
- 최종 갱신: 2026-09-23
- 현재 단계: **단계 7 (노드 종류 추상화) 완료. v2 시작.** `background` 상태 추가.
...
- **상태를 노드 테두리로 보여 준다**(3px): 대기=흰색, 작업 중=파랑, 입력 대기=주황+깜빡임, 완료=초록+느린 깜빡임, 세션 없음=회색 점선. 확인하면 깜빡임이 멈춘다. 선택 표시는 테두리를 뺏지 않도록 바깥 링(outline)으로 그린다.
- **테마를 바꿀 수 있다**: 배경 계열 5종(다크·미드나이트·그래파이트·포레스트·라떼) + 강조색 7종. 화면(CSS 변수)과 터미널(xterm `theme`)에 함께 반영된다. **상태 색은 테마와 무관하게 고정**이다.
...
- **백그라운드 작업을 따로 보여 준다**(SPEC 8.1): 에이전트가 백그라운드 작업을 기다리는 동안 보라 테두리 + 느린 숨쉬기(`⧗ 백그라운드`). 주황(`입력 대기`)과 섞이지 않는다 — 하나는 내가 할 일이 있다는 뜻이고 하나는 없다는 뜻이다. unseen·알림·`J` 순회에서는 뺀다.
- `npm run app:update`로 패키징 앱에 코드를 반영한다. **앱이 돌고 있으면 교체하지 않고 멈춘다** — 강제 종료하지 않는다.
...
> ⚠️ **앱을 고칠 때는 `npm run dev:isolated`를 쓴다.** 그냥 `npm run dev`는 패키징 앱과
> 같은 tmux 소켓·같은 `workspace.json`을 써서, 사용자가 실제로 작업 중인 세션과 간섭한다.
> 그리고 **실행 중인 앱은 묻지 않고 끄지 않는다** — 강제 종료해도 tmux 세션은 살아남지만,
> 사용자의 화면을 임의로 닫는 일이다.
...
### 다음 방향: v2 (SPEC 15~20장)
**설계 확정, 구현 전.**

목적이 한 문장으로 정해졌다: **"에이전트가 한 일과 그 결과를 잃지 않게 붙들어 두는 것."**
코드 뷰어·DB 그래프는 **목표가 아니다.** MVP가 "어느 에이전트가 날 기다리나"를 풀었다면,
v2는 "그 에이전트가 **무엇을** 했나"를 푼다.

가장 중요한 발견: **필요한 데이터가 이미 전부 디스크에 있다.**
`~/.claude/projects/<프로젝트>/<session_id>.jsonl` (훅의 `transcript_path`가 가리키는 그 파일)에
프롬프트·도구 호출·바뀐 파일·`ai-title`이 전부 들어 있다. 실측으로 확인했다:

- `Edit` → `file_path`, `old_string`, `new_string` — **이것 자체가 diff다**
- `Write` → `file_path`, `content`
- `file-history-delta.trackingPath` → 바뀐 파일 경로
- `ai-title` → Claude Code가 붙인 세션 제목 (노드 제목 자동 채우기에 쓸 수 있다)

즉 작업 기록은 **수집이 아니라 읽기·색인 문제**다. 훅으로 따로 모을 필요가 없고, 앱 밖에서
돌린 과거 세션도 읽힌다. 단, **비공식 내부 포맷이라 깨질 수 있다** — 모르는 타입은 무시하고
못 읽어도 앱은 그대로 동작해야 한다(R14).

정해진 것:
- 변경 기준은 **"마지막으로 확인한 지점 이후"**. 시각이 아니라 transcript의 `messageId`로 잡는다
- `unseen`(주의, 8.1)과 `unreviewed`(검토, 17장)를 **분리한다.** 포커스했다고 코드 변경을 다 읽은 건 아니다
...
### 7-6. 백그라운드 작업 대기를 "입력 대기"로 보여 주고 있었다 ⭐

에이전트가 백그라운드 작업(빌드·점검 등)을 기다리는 동안 노드가 **주황색 `입력 대기`**로
떴다. 사용자가 할 일은 아무것도 없는데 "너를 기다린다"고 거짓말을 한 것이다. 노드가 여러
개일 때 정작 진짜로 나를 기다리는 노드를 못 찾게 된다.

**어디서 오는지 실측으로 찾았다.** 사용자 노드의 상태 파일(`~/.session-canvas/status/*.json`)을
그대로 읽어 보니 `Stop` 페이로드에 이런 게 있었다:

```json
"hook_event_name":"Stop","stop_hook_active":false,"background_tasks":[],"session_crons":[]
```

`background_tasks`가 **실제로 온다.** 문서를 찾지 않고 디스크를 본 것이 빨랐다(4-2와 같은 교훈).

고친 방식:
- `Stop`/`StopFailure`에 작업이 남아 있으면 `done`이 아니라 새 상태 `background`
- 배열이 아니면 "모른다"로 보고 예전처럼 `done` — **"0개"라고 단정하지 않는다**
- 진짜 주황의 출처는 `Stop` **다음에 오는 유휴 `Notification`**이었다. 그건 `background_tasks`를
  안 들고 오므로 `StatusWatcher`가 노드별 개수를 **기억**하고, 기억한 값이 0보다 크면 그
  `waiting`을 `background`로 덮는다(`resolveState`)
- **`PermissionRequest`는 통과시킨다.** 백그라운드 중에도 권한은 사람이 허락해야 하고 숨기면
  노드가 영원히 멈춘다. 이 한 가지만 "놓치는 것보다 한 번 더 알린다" 원칙을 거스르므로
  SPEC 8.2에 이유를 적어 뒀다

**아직 확인 못 한 것**: `Notification` 페이로드에 `background_tasks`가 들어오는지. 들어온다면
기억 대신 그 값을 직접 쓰는 쪽이 정확하다. 노드가 그 상태로 바뀌는 순간 상태 파일을 보면 된다.

### 7-7. `timeout 300`을 씌워서 점검을 죽이고 "출력이 없다"고 읽었다

점검 4종을 한 번에 돌리면서 `timeout 300 npm run verify:... | grep ... | tail`을 썼다. 결과가
전부 빈 줄이었다. 스크립트가 300초보다 오래 걸려서 죽었고, `grep | tail`이 버퍼링하므로
죽는 순간 **아무것도 흘러나오지 않았다.** 개별로 시간을 넉넉히 주고 돌리니 전부 정상 출력됐다.

교훈이 반복된다: **점검이 이상하면 앱보다 점검(과 그걸 감싼 내 명령)을 먼저 의심한다.**
그리고 **점검 출력은 파이프로 자르지 말고 파일에 통째로 남긴다.** 자른 출력으로 판정하면
실패 이유가 사라진다.

### 7-8. 같은 타입이 두 곳에 정의돼 있었다

`StatusChange`에 필드 하나(`backgroundTasks`)를 넣는데 **두 곳**을 고쳐야 했다 —
`src/main/status/StatusWatcher.ts`와 `src/shared/ipc.ts`에 같은 이름으로 각각 있다.
한쪽만 고치면 `typecheck`가 "이 속성은 없다"와 "이 속성이 빠졌다"를 **동시에** 뱉어서
한참 헷갈렸다. 기존 중복이라 이번엔 양쪽에 같이 넣기만 했다. 합치지는 않았다.
...
원인은 단순했다. `zoomToNode`가 `setCenter(..., { zoom: 1 })`로 **배율을 고정**하고 있었다. 노드가 화면보다 크면 당연히 잘린다.

고치는 과정에서 훨씬 큰 것이 나왔다. `fitView`로 바꿨더니 **아무 일도 일어나지 않았고**, `Cmd+0`·`Cmd+1`·⤢ 버튼이 전부 죽어 있었다. 추적하면서 **네 번 틀렸다**:

1. "내 패치 탓이다" → 변경을 전부 stash하고 재현해 보니 **그대로 재현**. 내 탓이 아니었다
2. "대량 출력 후 렌더러가 막힌 것이다" → `Runtime.evaluate`는 멀쩡히 응답했다. 렌더러는 살아 있었다
3. "줌아웃 시 터미널 10개를 언마운트하느라 막힌다" → 줌인도 똑같이 막혔고 배율은 아예 안 바뀌었다
4. "`fitView`가 문제다" → `setCenter`로 바꿔도 같았다

진짜 원인: **창이 가려져 있으면(`document.visibilityState === 'hidden'`) Chromium이 `requestAnimationFrame`을 멈춘다.** 애니메이션이 들어간 뷰포트 이동은 d3 전환을 쓰므로 **한 프레임도 돌지 못하고 영영 완료되지 않는다.** `duration: 0`으로 바꾸자 즉시 정확한 좌표로 이동했다.

- 점검 스크립트가 띄우는 창은 보통 가려져 있다 → 그래서 개발 중에만 보였다
- 그런데 **실제 사용자에게도 문제가 된다.** 알림을 클릭해 노드로 줌인하는 경로(SPEC 8.6)가 정확히 "창이 가려진 상태"다
- 그래서 `viewportDuration()`을 만들어 보이지 않을 때는 애니메이션을 끈다

덤으로 알게 된 것: React Flow의 `fitView`는 내부 노드 변경 큐를 거쳐야 완료되는데, 리사이즈를 고치려고 노드에 `measured`를 넘기면서(6-4) 변경이 0건이 되어 그 경로가 끊겼다. 그래서 `fitView`를 버리고 배율을 직접 계산한다(`zoomToFit`, 단위 테스트 있음).

**교훈**: 증상이 "아무 일도 안 일어남"일 때는 내가 부른 함수가 *완료되었는지*부터 확인해야 한다. 반환값 로그 한 줄이 네 번의 추측보다 빨랐다.

### 7-2. 합성 핀치(Ctrl+휠)가 특정 상태에서 응답하지 않는다 (미해결)
노드 10개에 대량 출력을 부은 뒤 CDP로 `Ctrl+휠`을 보내면 ack가 오지 않는다. 클릭·이동은 수십 ms로 정상이고 배율도 안 바뀐다. 원인을 확정하지 못했다.
→ `check-zoom`에서는 합성 핀치를 쓰지 않고 배율을 직접 지정한다. 핀치 동작 자체는 `check-canvas`(노드 3개)가 검증한다. **실제 트랙패드 핀치는 다른 경로이므로 사용자 확인이 필요하다.**

### 7-3. 첫 입력이 키보드면 먹지 않는다
점검에서 핀치를 걷어냈더니 `Cmd+0`이 안 먹었다. 그 세션의 **첫 입력 이벤트**가 키보드였기 때문이다. 빈 캔버스를 한 번 클릭해 두면 그 뒤로는 정상이다(실제 사용자도 창을 클릭하고 쓴다).

### 6-4. 노드 리사이즈가 제멋대로 튀었다 ⭐⭐
사용자 신고: 손잡이를 잡기도 어렵고, 잡는 순간 노드가 최소 크기로 튀며 마우스가 순간이동하고, 늘리려면 한참 끌어야 반응했다.

원인이 **두 개**였다.

1. `adoptUserNodes`는 우리가 넘긴 노드 객체의 `measured`만 보존한다. 스토어가 바뀔 때마다 노드 객체를 새로 만들면서 `measured`를 안 넘겨서, 리사이즈 시작 시 `node.measured.width ?? 0` → **0**이 되고 `minWidth`로 보정돼 최소 크기로 튀었다. → `measured`를 함께 넘겨 해결.

2. **이게 진짜 원인이었다.** React Flow `ResizeControl`의 effect 의존성에 `onResize`/`onResizeEnd`가 들어 있다. 우리가 **인라인 화살표 함수**를 넘겨서 매 렌더마다 새 함수가 됐고, 드래그 중 `updateNode` → 재렌더 → effect 재실행 → `resizer.destroy()` + `update()`. `update()`는 `startValues`를 초기화하므로 **매 프레임 기준점이 리셋**됐다. 측정값:
   ```
   전: 640×420 → 654×320 → 720×324 → 704×324 → 618×328   (무작위)
   후: 640×420 → 690×454 → 740×488 → 790×522 → 940×624   (커서와 1:1)
   ```
   → `useCallback`으로 콜백을 고정해 해결.

**놓친 이유**: 내 검사가 *줄이기*만, 그것도 *최종값*만 봤다. 늘리기는 아예 안 봤고, 드래그 중 추적도 안 봤다. 이제 둘 다 본다 — "늘리기: 드래그 중에도 커서를 따라온다".

교훈: **끝값만 맞으면 통과하는 검사는 중간 과정이 엉망인 것을 못 잡는다.**

### 6-6. `asar extract-file`이 저장소 루트에 번들을 떨궜다 ⭐
패키징 앱에 변경이 들어갔는지 보려고 `npx asar extract-file <asar> <경로>`를 썼는데, 이 명령은
stdout으로 내보내지 않고 **현재 폴더에 파일을 만든다.** 그래서 1.7MB짜리 압축 렌더러 번들
`index-*.js`가 저장소 루트에 생겼고, `eslint .`가 그걸 검사하다 **1GB·98% CPU로 사실상 멈췄다.**
같은 명령을 여러 번 돌려 eslint 프로세스가 6개까지 쌓였다.

→ 떨어진 파일을 지우니 lint가 3.5초로 돌아왔다. `eslint.config.mjs`의 무시 패턴도
`'**/dist'` → `'**/dist/**'` 로 고쳤다(전자는 디렉터리 자체만 무시하고 안쪽은 계속 훑는다).

교훈: **패키징 앱 확인은 파일을 꺼내지 말고 앱을 띄워서 CDP로 본다.** 실제로 그게 더 확실하다
— 새 빌드에 CSS 규칙이 들어갔는지 `document.styleSheets`로 조회해서 확인했다.

### 6-5. 깜빡이는 요소의 색을 정확한 값으로 검사하면 안 된다
상태 테두리 검사가 `rgb(255, 157, 60)`을 기대했는데 `rgb(255, 180, 101)`이 나와 실패했다. 애니메이션이 도는 중이라 **두 색 사이의 중간값**을 읽은 것이다(애니메이션 자체는 정상이었다).
→ 애니메이션 이름 + 색 계열(R>G>B 등)로 판정한다.
