...
**설계 확정, 구현 전.** 캔버스를 본체로 삼아 터미널 외의 노드(코드 뷰어, DB 관계 그래프, MCP 도구)를 올리고, Figma처럼 프레임·레이어·노트로 묶어 관리한다.

정해진 것:
- **모드 분리** — 캔버스 모드(한 글자 단축키, 터미널 입력 막힘) ↔ 노드 모드(더블클릭 진입, 모든 키 전달, `Cmd+.`로 탈출). `Esc`는 Claude Code가 써서 전환키로 못 쓴다
- **기반 공사부터** — 단계 7(노드 종류 추상화) → 8(캔버스 편집) → 9~11(도구 노드)
- **동시 비교할 것만 노드** — 파일 트리·검색·레이어는 고정 패널

단계 7의 완료 기준이 **"겉보기 변화 없음 + 기존 점검 전부 통과"** 인 점이 중요하다. 순수 리팩터링이라 회귀만 없으면 성공이다.

착수 전 확인할 것: R10(프레임 중첩과 리사이즈 로직), R11(레이어 z-order). 리사이즈는 이미 한 번 크게 깨졌다(6-4).
